		...
		if (round == 1) {
			...
			for (City city : allCities) {
				if (!myVehicleCities.contains(city)) {
					double diff = Math
							.abs((city.distanceTo(previous.pickupCity) + previous.pickupCity
									.distanceTo(previous.deliveryCity))
									* oppVehicles.get(0).getCostPerKm()
									- oppBid);
					if(diff < costDiff){
						costDiff = diff;
						predictCity = city;
...
//		}
	...
	public List<Plan> plan(List<Vehicle> vehicles, TaskSet tasks) {
		...
		CentralizedPlan selectedPlan = pdpAlg.getBestPlan();

		System.out.println(myPlan.getBestPlan().cost() +"VS" + selectedPlan.cost());
		
		//CentralizedPlan candidatePlan = myPlan.getBestPlan();
		
//		System.out.println(candidatePlan.getTaskNum()+ " " + selectedPlan.getTaskNum());
//		if(candidatePlan.cost() < selectedPlan.cost()){
//			for(MyVehicle vehicle:myVehicles){
//				plans.add(makePlan(vehicle, selectedPlan.getVehicleActions().get(vehicle),tasks));
//			}
//		}
//		else{
		...
		for(Action action:linkedList){
			if(action.type == Type.PICKUP){
				...
				for(Task currentTask:tasks){
					if(currentTask.pickupCity == action.currentTask.pickupCity 
						&& currentTask.deliveryCity == action.currentTask.deliveryCity){
