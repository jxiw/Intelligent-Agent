...
public class DeliberativeTemplate implements DeliberativeBehavior {
					...
					for(int i=0;i<size;i++){
						...
						if(curentTaskSign.charAt(i) == '0'){
							...
							if(updatedWeight <= capacity){
								...
								State childState = new State(currentState,possibleTask, departure, childCost, new String(childSign),updatedWeight,true);
							...
							State childState = new State(currentState,possibleTask, destination, childCost, new String(childSign),updatedWeight,false);
							queue.add(childState);
						}
					}
				}
	...
	public Plan AStarPlan(City city,TaskSet tasks){
		...
		State startState = new State(null,null,city,0,intialTaskSign,0,true);
		PriorityQueue<State> queue = new PriorityQueue<State>(1,stateCompartor);
		queue.add(startState);
		int iterationNum=0;
					...
					for(int i=0;i<size;i++){
						if(currentTaskSign.charAt(i) == '0'){
							...
							if(updatedWeight <= capacity){
								...
								State childState = new State(currentState,possibleTask, departure, childCost, new String(childSign),updatedWeight,true);
								if(!searchStateSet.contains(childState)){
									queue.add(childState);
								}
							}
						}
