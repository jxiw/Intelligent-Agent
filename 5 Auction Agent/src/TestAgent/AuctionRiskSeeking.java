...
public class AuctionRiskSeeking implements AuctionBehavior {
	...
	public Long askPrice(Task task) {
		long cost = (long) (
				myplan.margCostEstim(task) * (1-Math.pow(Math.E, -this.numBids*this.ETA))
				+ myplan.margAvgCostEstim(generateRandTasks()) * Math.pow(Math.E, -this.numBids*this.ETA));
		long opponentcost = (long) (
				opponentplan.margCostEstim(task) * (1-Math.pow(Math.E, -this.numBids*this.ETA))
