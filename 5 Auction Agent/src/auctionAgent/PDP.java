...
public class PDP {
	...
	public void SLSAlgorithmWithInitPlan(long allowedTime, CentralizedPlan plan) {
		...
		for (int i = 0; i < iterationNum; i++) {
			...
			// To prevent time out, after deadline the best solution will be
			// found
	...
	public ArrayList<CentralizedPlan> ChooseNeighbours(CentralizedPlan oldPlan) {
		...
		for (MyVehicle exchangeVehicle : vehicles) {
			for (MyVehicle currentVehicle : vehicles) {
				...
				if (currentVehicle != exchangeVehicle && vehicleActions.size() > 0) {
	...
	public List<CentralizedPlan> changingVehicle(CentralizedPlan oldPlan, MyVehicle v1, MyVehicle v2) {
		...
		Task startTask = newPlan.getVehicleActions().get(v1).get(0).currentTask;// get
																				// vehicle1
																				// startValue
		...
		LinkedList<Action> stateListV2 = newPlan.getVehicleActions().get(v2);// get
																				// vehicle2
																				// nextStateList
		stateListV2.addFirst(pickupAction);// set firstState before
											// nextStateList
