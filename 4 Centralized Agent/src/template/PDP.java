...
public class PDP {
					...
					if (currentVehicle != exchangeVehicle && nextState != null && nextState.size() > 0) {
						for (int i = 0; i < oldPlan.getNextState().get(exchangeVehicle).size(); i++) {
							...
							if (exchangeState != null
									&& exchangeState.getCurrentTask().weight <= currentVehicle.capacity()) {
								List<CentralizedPlan> planList = changingVehicle(oldPlan, exchangeVehicle,
										currentVehicle);
	...
	public List<CentralizedPlan> changingVehicle(CentralizedPlan oldPlan, Vehicle v1, Vehicle v2) {
		...
		State startStateV1 = newPlan.getNextState().get(v1).get(0);// get
																	// vehicle1
																	// startValue
		LinkedList<State> nextState1 = newPlan.getNextState().get(v1);// get
																		// vehicle1
																		// nextStateList
		// slove bug
		...
		// nextStateList.remove(0);//remove firstState contain corresponding
		// delivery State

		// add v2 nextState
		// State startStateV2 = newPlan.getNextState().get(v2).get(0);//get
		// vehicle2 startValue
		LinkedList<State> stateListV2 = newPlan.getNextState().get(v2);// get
																		// vehicle2
																		// nextStateList
		...
		stateListV2.addFirst(startStateV1);// set firstState before
											// nextStateList
				...
				for (int i = 0; i < nextState.size() - 1; i++) {
					...
					if (preState.isPickup() == true && postState.isPickup() == true) {
						...
						cost += preState.getCurrentTask().deliveryCity
								.distanceTo(postState.getCurrentTask().deliveryCity) * v1.costPerKm();
					}
